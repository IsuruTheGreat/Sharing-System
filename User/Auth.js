...
async function sendOtp() {
  ...
  // ── Secret PIN bypass ────────────────────────────────────────────────────
  // If the owner types the PIN instead of a phone number, skip OTP entirely
  // and log in directly as owner. Change the PIN below to whatever you want.
  const OWNER_PIN = CONFIG.APP.OWNER_PIN;
  if (phone === OWNER_PIN) {
    activeSession = {
      token:    "owner-pin-bypass",
      phone:    CONFIG.APP.ADMIN_WHATSAPP,
      name:     "Owner",
      role:     "owner",
      projects: [],
      isNew:    false
